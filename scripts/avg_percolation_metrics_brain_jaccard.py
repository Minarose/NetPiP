...
try:
    ...
    raise RuntimeError(
        "nilearn is required for brain plots. Install via "
        "`pip install -r analysis/giant_component_avg_nonexcluded/requirements.txt`."
    ) from exc
...
from plot_pip_surfaces import (  # noqa: E402
...
def load_node_p(conv_path: Path) -> np.ndarray:
    ...
    # cannot read. plot_pip_surfaces.load_pip_any handles both classic + v7.3.
    ...
    *,
    ...
    if venn2 is None:
        raise RuntimeError(
            ...
            "Install via `pip install -r analysis/giant_component_avg_nonexcluded/requirements.txt`."
        )
    A = set(a)
    B = set(b)
...
def main() -> None:
    ...
    p.add_argument(
        "--avg-conv-mat",
        default=str(repo_root / "data" / "PSI_broadband_MEG_mats" / "results_converge_giant_avg_nonexcluded" / "AVG_broadband_psi_adj_giant75_nonexcluded_ConvHW.mat"),
    ...
    p.add_argument("--out-dir", default=str(repo_root / "results" / "avg_metric_overlap"), help="Output directory.")
    p.add_argument(
        "--top-n",
        type=int,
        default=None,
