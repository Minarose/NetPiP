...
(plot_pip_surfaces.plot_tauS6_for_set): same get_tilt_peak_order_amplitude ranking,
...
from plot_pip_surfaces import crop_longest_non_nan_block, get_tilt_peak_order_amplitude, load_pip_any  # noqa: E402
...
def main() -> None:
    ...
    p.add_argument(
        ...
        default=True,
        help="Match plot_pip_surfaces default (default: clip).",
    )
    p.add_argument(
        "--reverse",
        action="store_true",
    ...
    p.add_argument(
        "--out-matrix-csv",
        default=str(
            LEG_ROOT / "results" / "per_subject_pip2d_order_matlab_noheader.csv"
        ),
    ...
    p.add_argument(
        "--out-txt",
        default=str(
            LEG_ROOT / "results" / "AVG_giant75_PiP_2d_label_order_matlab_nodes.txt"
        ),
    )
    p.add_argument(
        "--out-csv",
        default=str(
            LEG_ROOT / "results" / "AVG_giant75_PiP_2d_label_order_matlab.csv"
        ),
    )
    args = p.parse_args()
