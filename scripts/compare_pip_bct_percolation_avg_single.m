% compare_pip_bct_percolation_avg_single.m
% -------------------------------------------------------------------------
...
%
% This is the "average analysis" analogue of compare_pip_bct_percolation_giant75.m,
...
avgConvMat = fullfile(repoRoot, 'data', 'PSI_broadband_MEG_mats', 'results_converge_giant_avg_nonexcluded', ...
...
clusterCsv = fullfile(repoRoot, 'results', 'consensus_5pct', 'avg_giant_nonexcluded_top_cluster_nodes_labeled.csv');
if ~isfile(clusterCsv)
    error("Missing cluster CSV:\n%s\nRun the avg cluster + label step first (see analysis bundle README).", clusterCsv);
end
